"use strict";(()=>{function d(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function b(e){try{let t=new URL(e);if(t.protocol==="http:"||t.protocol==="https:")return e}catch{}return""}function p(e,t){document.getElementById("modal-overlay")?.remove();let r=document.createElement("div");r.id="modal-overlay",r.className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50";let a=document.createElement("div");a.className="bg-gray-800 border border-gray-700 rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl";let n=document.createElement("p");n.className="text-white text-center mb-6 text-base leading-relaxed",n.textContent=e,a.appendChild(n);let l=document.createElement("div");if(l.className="flex gap-3 justify-center",t){let c=document.createElement("button");c.className="flex-1 py-2.5 rounded-xl bg-gray-600 hover:bg-gray-500 text-white font-semibold transition",c.textContent="\u041E\u0442\u043C\u0435\u043D\u0430",c.onclick=()=>r.remove(),l.appendChild(c);let x=document.createElement("button");x.className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-semibold transition",x.textContent="\u041F\u043E\u0434\u0442\u0432\u0435\u0440\u0434\u0438\u0442\u044C",x.onclick=()=>{r.remove(),t()},l.appendChild(x)}else{let c=document.createElement("button");c.className="px-10 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition",c.textContent="OK",c.onclick=()=>r.remove(),l.appendChild(c)}a.appendChild(l),r.appendChild(a),r.addEventListener("click",c=>{c.target===r&&r.remove()}),document.body.appendChild(r)}function G(e){let t=Math.floor((Date.now()-e)/6e4);if(t<1)return"\u0442\u043E\u043B\u044C\u043A\u043E \u0447\u0442\u043E";if(t<60)return`${t} \u043C\u0438\u043D. \u043D\u0430\u0437\u0430\u0434`;let r=Math.floor(t/60);return r<24?`${r} \u0447. \u043D\u0430\u0437\u0430\u0434`:`${Math.floor(r/24)} \u0434. \u043D\u0430\u0437\u0430\u0434`}function A(e,t){let r=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),a=URL.createObjectURL(r),n=document.createElement("a");n.href=a,n.download=t,n.click(),URL.revokeObjectURL(a)}var D="clu_packs",K="clu_character_sets",N="clu_saves";function q(){try{let e=JSON.parse(localStorage.getItem(D)??"[]");return Array.isArray(e)?e:[]}catch{return[]}}function C(e){localStorage.setItem(D,JSON.stringify(e))}function I(){try{let e=JSON.parse(localStorage.getItem(K)??"[]");return Array.isArray(e)?e:[]}catch{return[]}}function M(e){localStorage.setItem(K,JSON.stringify(e))}function f(){try{let e=JSON.parse(localStorage.getItem(N)??"[]");return Array.isArray(e)?e:[]}catch{return[]}}function P(e){let t=f(),r=t.findIndex(l=>l.id===e.id),a=e.unlockedUpTo>=0?e.characterSet.characters[e.unlockedUpTo]?.name??"":"",n={id:e.id,playerName:e.playerName,packTitle:e.pack.title,charName:a,questionProgress:`${e.currentIndex+1} / ${e.shuffledQuestions.length}`,totalStars:e.totalStars,savedAt:Date.now(),state:e};r>=0?t[r]=n:t.unshift(n),localStorage.setItem(N,JSON.stringify(t))}function H(e){let t=f().filter(r=>r.id!==e);localStorage.setItem(N,JSON.stringify(t))}async function V(){try{let t=await(await fetch("packs/index.json")).json();return Promise.all(t.map(r=>fetch(`packs/${r}`).then(a=>a.json())))}catch{return[]}}async function Y(){try{let t=await(await fetch("characters/index.json")).json();return Promise.all(t.map(r=>fetch(`characters/${r}`).then(a=>a.json())))}catch{return[]}}var i=null,j=null,y=0,h=()=>{},Z=[],ee=[];function te(e){h=e}async function re(){[Z,ee]=await Promise.all([V(),Y()])}function ae(){return f().length>0}function fe(){return[...Z,...q()]}function he(){return[...ee,...I()]}function se(e){let t=fe(),r=he(),a=t.length>0&&r.length>0;if(e.innerHTML=`
      ...
      <div class="w-full max-w-md space-y-5">
        ...
        <div>
          ...
          ${t.length>0?`<select id="sel-pack" class="w-full bg-gray-800 text-white rounded-xl px-4 py-3 border border-gray-700 focus:outline-none focus:border-indigo-500">
                ${t.map((n,l)=>`<option value="${l}">${d(n.title)} (${n.questions.length} \u0432\u043E\u043F\u0440. \xB7 ${n.starsPerCorrect}\u2605)</option>`).join("")}
        ...
        <div>
          ...
          ${r.length>0?`<select id="sel-chars" class="w-full bg-gray-800 text-white rounded-xl px-4 py-3 border border-gray-700 focus:outline-none focus:border-indigo-500">
                ${r.map((n,l)=>`<option value="${l}">${d(n.title)} (${n.characters.length} \u043F\u0435\u0440\u0441.)</option>`).join("")}
  ...
  `,a){let n=e.querySelector("#sel-pack"),l=e.querySelector("#sel-chars"),c=()=>{let x=t[parseInt(n.value)],v=r[parseInt(l.value)],u=e.querySelector("#compat-warning");if(!x||!v){u.innerHTML="";return}let g=x.questions.length*x.starsPerCorrect,m=[...v.characters].sort((U,be)=>U.cost-be.cost),k=m[m.length-1]?.cost??0,z=m.filter(U=>U.cost<=g).length;z===0?u.innerHTML=`<div class="mt-2 px-3 py-2 bg-red-950 border border-red-800 rounded-xl text-red-300 text-xs">
          \u26D4 \u041D\u0435\u043B\u044C\u0437\u044F \u043E\u0442\u043A\u0440\u044B\u0442\u044C \u043D\u0438 \u043E\u0434\u043D\u043E\u0433\u043E \u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0430 \u2014 \u043C\u0430\u043A\u0441. ${g}\u2605, \u0430 \u0441\u0430\u043C\u044B\u0439 \u0434\u0435\u0448\u0451\u0432\u044B\u0439 \u0441\u0442\u043E\u0438\u0442 ${m[0]?.cost??0}\u2605
        </div>`:g<k?u.innerHTML=`<div class="mt-2 px-3 py-2 bg-yellow-950 border border-yellow-800 rounded-xl text-yellow-300 text-xs">
          \u26A0 \u041C\u043E\u0436\u043D\u043E \u043E\u0442\u043A\u0440\u044B\u0442\u044C ${z} \u0438\u0437 ${m.length} \u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0435\u0439 (\u043C\u0430\u043A\u0441. ${g}\u2605, \u043D\u0443\u0436\u043D\u043E ${k}\u2605)
        </div>`:u.innerHTML=`<div class="mt-2 px-3 py-2 bg-green-950 border border-green-800 rounded-xl text-green-400 text-xs">
          \u2713 \u0412\u0441\u0435 ${m.length} ${ye(m.length,"\u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436","\u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0430","\u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0435\u0439")} \u043E\u0442\u043A\u0440\u044B\u0432\u0430\u0435\u043C\u044B (\u043C\u0430\u043A\u0441. ${g}\u2605 \u043F\u0440\u0438 \u043D\u0443\u0436\u043D\u044B\u0445 ${k}\u2605)
        </div>`};n.addEventListener("change",c),l.addEventListener("change",c),c(),e.querySelector("#btn-start")?.addEventListener("click",()=>{let x=e.querySelector("#inp-name").value.trim(),v=parseInt(n.value),u=parseInt(l.value),g=Math.max(0,parseInt(e.querySelector("#inp-timer").value)||0);ke(t[v],r[u],g,x)})}e.querySelector("#btn-back")?.addEventListener("click",()=>h("home"))}function ye(e,t,r,a){let n=e%100;if(n>=11&&n<=14)return a;switch(e%10){case 1:return t;case 2:case 3:case 4:return r;default:return a}}function we(e){let t=[...e];for(let r=t.length-1;r>0;r--){let a=Math.floor(Math.random()*(r+1));[t[r],t[a]]=[t[a],t[r]]}return t}function ke(e,t,r,a){let n=[...t.characters].sort((l,c)=>l.cost-c.cost);i={id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),playerName:a,pack:e,characterSet:{...t,characters:n},shuffledQuestions:we(e.questions),currentIndex:0,totalStars:0,timerSeconds:r,unlockedUpTo:-1},P(i),h("game"),Q()}function ne(e){let t=f().find(r=>r.id===e);t&&(i=t.state,h("game"),Q())}function Q(){if(!i)return;w();let e=document.getElementById("screen-game"),t=i.characterSet.characters,r=i.unlockedUpTo>=0?t[i.unlockedUpTo]??null:null,a=t[i.unlockedUpTo+1]??null,n=i.shuffledQuestions[i.currentIndex],l=r?.cost??0,c=a?.cost??l,x=a?Math.min(100,Math.max(0,(i.totalStars-l)/(c-l)*100)):100,v=a?Math.max(0,a.cost-i.totalStars):0,u=(g,m="")=>{let k=g?b(g):"";return k?`<img src="${k}" alt="" class="${m}" />`:null};e.innerHTML=`
    <div class="h-screen flex flex-col bg-gray-950">
      ...
      <div class="flex flex-1 min-h-0">
        ...
        <div class="flex-1 flex flex-col items-center justify-center gap-5 p-8 overflow-y-auto">
          ...
          ${i.timerSeconds>0?`<div id="timer-display" class="text-6xl font-black text-white leading-none">${i.timerSeconds}</div>`:""}
          ...
          <div id="answer-block" class="hidden w-full max-w-xl flex flex-col gap-4">
            ...
            <div class="flex gap-3">
              ...
              <button id="btn-correct"
                ...
                <span class="text-2xl">\u2713</span>
                <span>\u041F\u0440\u0430\u0432\u0438\u043B\u044C\u043D\u043E +${i.pack.starsPerCorrect}\u2605</span>
              </button>
            </div>
          </div>
        ...
        <div class="w-[27%] flex-shrink-0 bg-gray-900 border-l border-gray-800 flex flex-col">
          ...
          <div class="px-4 py-3 border-b border-gray-800 text-center flex-shrink-0">
            ...
            <div class="text-yellow-400 font-black text-4xl">\u2605 ${i.totalStars}</div>
          </div>
          ...
          <div class="flex-1 flex flex-col items-center px-4 pt-3 pb-2 min-h-0">
            ...
            <div class="flex-1 w-full min-h-0 flex items-center justify-center">
              ${r?(()=>{let g=r.imageUrl?b(r.imageUrl):"";return g?`<img src="${g}" alt="" class="max-h-full max-w-full object-contain rounded-3xl" />`:'<div class="w-full h-full bg-gray-700 rounded-3xl flex items-center justify-center text-7xl">\u265F</div>'})():'<div class="w-full h-full bg-gray-800 rounded-3xl flex items-center justify-center text-6xl opacity-40">\u{1F512}</div>'}
            </div>
          ...
          <div class="flex-1 flex flex-col items-center px-4 pt-2 pb-3 border-t border-gray-800 min-h-0">
            ...
            <div class="flex-1 w-full min-h-0 flex items-center justify-center">
              ${a?(()=>{let g=a.imageUrl?b(a.imageUrl):"";return g?`<img src="${g}" alt="" class="max-h-full max-w-full object-contain rounded-3xl opacity-35 grayscale" />`:'<div class="w-full h-full bg-gray-800 rounded-3xl flex items-center justify-center text-7xl opacity-35">\u{1F512}</div>'})():'<div class="w-full h-full bg-gray-800 rounded-3xl flex items-center justify-center text-8xl">\u{1F3C6}</div>'}
            </div>
            ...
            <div class="flex gap-2">
              ...
              <div class="relative flex-1 h-11 rounded-xl overflow-hidden bg-gray-800 border border-gray-700">
                <div class="absolute inset-0 bg-gradient-to-r from-indigo-600 to-violet-600 transition-all duration-500"
                  style="width:${Math.round((i.currentIndex+1)/i.shuffledQuestions.length*100)}%"></div>
                <span class="absolute inset-0 flex items-center justify-center text-sm font-semibold text-white drop-shadow">
                  ${i.currentIndex+1} / ${i.shuffledQuestions.length}
                </span>
              </div>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-4xl shadow-2xl flex flex-col max-h-[85vh]">
      ...
      <div class="flex items-center justify-between px-5 py-4 border-b border-gray-800">
        <div>
          ...
          <div class="text-xs text-gray-400 mt-0.5">
            \u041D\u0430\u0431\u0440\u0430\u043D\u043E: <span class="text-yellow-400 font-bold">\u2605 ${i.totalStars}</span>
            &nbsp;\xB7&nbsp;
      ...
      <div class="px-5 py-3 border-b border-gray-800">
        <div class="flex justify-between text-xs text-gray-500 mb-1.5">
          ...
          <span>${Math.round(Math.min(100,i.totalStars/(a||1)*100))}%</span>
        </div>
        <div class="w-full bg-gray-700 rounded-full h-2.5 overflow-hidden">
          <div class="bg-yellow-400 h-2.5 rounded-full transition-all"
            style="width:${Math.min(100,i.totalStars/(a||1)*100)}%"></div>
        </div>
      </div>

      <!-- Characters grid -->
      <div class="grid grid-cols-4 gap-4 p-5 overflow-y-auto flex-1">
        ${r.map((x,v)=>{let u=v<=i.unlockedUpTo;return`
  ...
  `,document.body.appendChild(n);let c=()=>{n.remove(),e&&t>0&&ie(t)};n.querySelector("#btn-close-overlay")?.addEventListener("click",c),n.addEventListener("click",x=>{x.target===n&&c()})}function ie(e){w(),y=e,F(),j=window.setInterval(()=>{y--,F(),y<=0&&(w(),document.getElementById("btn-show-answer")?.classList.add("hidden"),document.getElementById("answer-block")?.classList.remove("hidden"))},1e3)}function w(){j!==null&&(clearInterval(j),j=null)}function F(){let e=document.getElementById("timer-display");e&&(e.textContent=String(y),y<=5?e.className="text-6xl font-black text-red-500 leading-none":y<=10?e.className="text-6xl font-black text-yellow-400 leading-none":e.className="text-6xl font-black text-white leading-none")}function W(e){if(i){if(w(),e){i.totalStars+=i.pack.starsPerCorrect;let t=i.characterSet.characters,r=null;for(;i.unlockedUpTo+1<t.length&&i.totalStars>=t[i.unlockedUpTo+1].cost;)i.unlockedUpTo++,r=t[i.unlockedUpTo];if(P(i),r){let n=i.unlockedUpTo>=t.length-1?()=>{i.currentIndex++,H(i.id),oe(),h("results")}:()=>X();$e(r,n);return}}X()}}function $e(e,t){let r=document.createElement("div");r.className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center z-50";let a=e.imageUrl?b(e.imageUrl):"";r.innerHTML=`
  ...
  `,document.body.appendChild(r),r.querySelector("#btn-unlock-ok")?.addEventListener("click",()=>{r.remove(),t()})}function X(){if(i){if(i.currentIndex++,i.currentIndex>=i.shuffledQuestions.length){H(i.id),oe(),h("results");return}P(i),Q()}}function oe(){if(!i)return;let e=document.getElementById("screen-results"),t=i.characterSet.characters,r=i.unlockedUpTo>=0?t.slice(0,i.unlockedUpTo+1):[],a=i.unlockedUpTo>=0?t[i.unlockedUpTo]??null:null,n=(l,c)=>{let x=l?b(l):"";return x?`<img src="${x}" alt="" class="${c}" />`:`<div class="${c} bg-gray-700 flex items-center justify-center text-4xl">\u2B50</div>`};e.innerHTML=`
    <div class="min-h-screen flex flex-col items-center justify-center p-6 bg-gray-950">
      ...
      <div class="text-gray-400 mb-8 text-sm">
        ${i.currentIndex} / ${i.shuffledQuestions.length} \u0432\u043E\u043F\u0440\u043E\u0441\u043E\u0432 \xB7 ${i.totalStars} \u0437\u0432\u0451\u0437\u0434 \u043D\u0430\u0431\u0440\u0430\u043D\u043E
      </div>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 text-center w-full max-w-xs mb-6">
        ...
        <div class="text-yellow-400 mt-1 text-sm">\u2605 ${i.totalStars}</div>
      </div>
          ...
          <div class="flex flex-wrap gap-2 justify-center">
            ${r.map(l=>`
              <div class="flex flex-col items-center bg-gray-900 border border-gray-800 rounded-xl p-2 w-20">
                ${n(l.imageUrl,"w-12 h-12 object-contain rounded-lg")}
  ...
  `,e.querySelector("#btn-home")?.addEventListener("click",()=>{i=null,h("home")})}var B=()=>{};function le(e){B=e}var o=de();function de(){return{packs:[],view:"list",editIdx:-1,pack:{title:"",questions:[],starsPerCorrect:3},expandedQ:-1}}function ce(e){o=de(),o.packs=q(),$(e)}function $(e){o.view==="list"?O(e):Ee(e)}function O(e){e.innerHTML=`
    <div class="min-h-screen flex flex-col bg-gray-950">
      ...
      <div class="flex-1 p-4 max-w-2xl mx-auto w-full">
        ${o.packs.length===0?'<div class="text-gray-500 text-center mt-20 text-sm">\u041D\u0435\u0442 \u043F\u0430\u043A\u0435\u0442\u043E\u0432. \u0421\u043E\u0437\u0434\u0430\u0439\u0442\u0435 \u043F\u0435\u0440\u0432\u044B\u0439!</div>':o.packs.map((t,r)=>`
  ...
  `,e.querySelector("#pe-back")?.addEventListener("click",()=>B("home")),e.querySelector("#pe-new")?.addEventListener("click",()=>{o.editIdx=-1,o.pack={title:"",questions:[],starsPerCorrect:3},o.view="edit",o.expandedQ=-1,$(e)}),e.querySelector("#pe-import")?.addEventListener("change",t=>{let r=t.target.files?.[0];if(!r)return;let a=new FileReader;a.onload=n=>{try{let l=JSON.parse(n.target?.result);if(!l.title||!Array.isArray(l.questions))throw new Error;o.packs.push(l),C(o.packs),O(e)}catch{p("\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 \u0444\u043E\u0440\u043C\u0430\u0442 \u0444\u0430\u0439\u043B\u0430")}},a.readAsText(r)}),e.querySelectorAll("[data-edit]").forEach(t=>{t.addEventListener("click",()=>{let r=parseInt(t.dataset.edit);o.editIdx=r,o.pack=JSON.parse(JSON.stringify(o.packs[r])),o.view="edit",o.expandedQ=-1,$(e)})}),e.querySelectorAll("[data-export]").forEach(t=>{t.addEventListener("click",()=>{let r=parseInt(t.dataset.export);A(o.packs[r],`${o.packs[r].title||"pack"}.json`)})}),e.querySelectorAll("[data-del]").forEach(t=>{t.addEventListener("click",()=>{let r=parseInt(t.dataset.del);p(`\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u043F\u0430\u043A\u0435\u0442 \xAB${d(o.packs[r].title)}\xBB?`,()=>{o.packs.splice(r,1),C(o.packs),O(e)})})})}function Ee(e){let t=o.pack;e.innerHTML=`
    <div class="min-h-screen flex flex-col bg-gray-950">

      <div class="flex items-center gap-3 px-5 py-4 bg-gray-900 border-b border-gray-800">
        <button id="pe-edit-back" class="text-gray-400 hover:text-white transition text-sm">\u2190 \u041D\u0430\u0437\u0430\u0434</button>
        <h1 class="text-xl font-bold flex-1">${o.editIdx===-1?"\u041D\u043E\u0432\u044B\u0439 \u043F\u0430\u043A\u0435\u0442":"\u0420\u0435\u0434\u0430\u043A\u0442\u0438\u0440\u043E\u0432\u0430\u0442\u044C \u043F\u0430\u043A\u0435\u0442"}</h1>
  ...
  `;let r=e.querySelector("#pe-title"),a=e.querySelector("#pe-stars");r.addEventListener("input",()=>{o.pack.title=r.value}),a.addEventListener("input",()=>{o.pack.starsPerCorrect=Math.max(1,parseInt(a.value)||1)}),e.querySelector("#pe-edit-back")?.addEventListener("click",()=>{o.view="list",o.packs=q(),$(e)}),e.querySelector("#pe-save")?.addEventListener("click",()=>{if(!o.pack.title.trim()){p("\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043D\u0430\u0437\u0432\u0430\u043D\u0438\u0435 \u043F\u0430\u043A\u0435\u0442\u0430");return}o.editIdx===-1?o.packs.push(o.pack):o.packs[o.editIdx]=o.pack,C(o.packs),o.view="list",$(e)}),e.querySelector("#pe-del-pack")?.addEventListener("click",()=>{p(`\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u043F\u0430\u043A\u0435\u0442 \xAB${d(o.pack.title)}\xBB?`,()=>{o.packs.splice(o.editIdx,1),C(o.packs),o.view="list",$(e)})}),e.querySelector("#pe-add-q")?.addEventListener("click",()=>{o.pack.questions.push({text:"",answer:"",imageUrl:""}),o.expandedQ=o.pack.questions.length-1,J(e),setTimeout(()=>{e.querySelector(`[data-qrow="${o.expandedQ}"]`)?.scrollIntoView({behavior:"smooth",block:"center"})},50)}),ue(e)}function xe(e){return e.length===0?'<div class="text-gray-500 text-center py-8 text-sm">\u041D\u0435\u0442 \u0432\u043E\u043F\u0440\u043E\u0441\u043E\u0432. \u041D\u0430\u0436\u043C\u0438\u0442\u0435 \xAB+ \u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C\xBB.</div>':e.map((t,r)=>{let a=o.expandedQ===r;return`
    ...
    `}).join("")}function J(e){let t=e.querySelector("#pe-qlist");t&&(t.innerHTML=xe(o.pack.questions)),ue(e)}function ue(e){e.querySelectorAll("[data-qtoggle]").forEach(t=>{t.addEventListener("click",r=>{if(r.target.hasAttribute("data-qdel"))return;let a=parseInt(t.dataset.qtoggle);o.expandedQ=o.expandedQ===a?-1:a,J(e)})}),e.querySelectorAll("[data-qdel]").forEach(t=>{t.addEventListener("click",r=>{r.stopPropagation();let a=parseInt(t.dataset.qdel);o.pack.questions.splice(a,1),o.expandedQ===a?o.expandedQ=-1:o.expandedQ>a&&o.expandedQ--,J(e)})}),e.querySelectorAll("[data-qtext]").forEach(t=>{t.addEventListener("input",()=>{o.pack.questions[parseInt(t.dataset.qtext)].text=t.value})}),e.querySelectorAll("[data-qans]").forEach(t=>{t.addEventListener("input",()=>{o.pack.questions[parseInt(t.dataset.qans)].answer=t.value})}),e.querySelectorAll("[data-qimg]").forEach(t=>{t.addEventListener("input",()=>{o.pack.questions[parseInt(t.dataset.qimg)].imageUrl=t.value})})}var s=ge();function ge(){return{sets:[],view:"list",editIdx:-1,set:{title:"",characters:[]},expandedC:-1}}function pe(e){s=ge(),s.sets=I(),E(e)}function E(e){s.view==="list"?R(e):Le(e)}function R(e){e.innerHTML=`
    <div class="min-h-screen flex flex-col bg-gray-950">
  ...
  `,e.querySelector("#ce-back")?.addEventListener("click",()=>B("home")),e.querySelector("#ce-new")?.addEventListener("click",()=>{s.editIdx=-1,s.set={title:"",characters:[]},s.view="edit",s.expandedC=-1,E(e)}),e.querySelector("#ce-import")?.addEventListener("change",t=>{let r=t.target.files?.[0];if(!r)return;let a=new FileReader;a.onload=n=>{try{let l=JSON.parse(n.target?.result);if(!l.title||!Array.isArray(l.characters))throw new Error;s.sets.push(l),M(s.sets),R(e)}catch{p("\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 \u0444\u043E\u0440\u043C\u0430\u0442 \u0444\u0430\u0439\u043B\u0430")}},a.readAsText(r)}),e.querySelectorAll("[data-edit]").forEach(t=>{t.addEventListener("click",()=>{let r=parseInt(t.dataset.edit);s.editIdx=r,s.set=JSON.parse(JSON.stringify(s.sets[r])),s.view="edit",s.expandedC=-1,E(e)})}),e.querySelectorAll("[data-export]").forEach(t=>{t.addEventListener("click",()=>{let r=parseInt(t.dataset.export);A(s.sets[r],`${s.sets[r].title||"characters"}.json`)})}),e.querySelectorAll("[data-del]").forEach(t=>{t.addEventListener("click",()=>{let r=parseInt(t.dataset.del);p(`\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u043D\u0430\u0431\u043E\u0440 \xAB${d(s.sets[r].title)}\xBB?`,()=>{s.sets.splice(r,1),M(s.sets),R(e)})})})}function Le(e){let t=s.set;e.innerHTML=`
    <div class="min-h-screen flex flex-col bg-gray-950">
  ...
  `;let r=e.querySelector("#ce-title");r.addEventListener("input",()=>{s.set.title=r.value}),e.querySelector("#ce-edit-back")?.addEventListener("click",()=>{s.view="list",s.sets=I(),E(e)}),e.querySelector("#ce-save")?.addEventListener("click",()=>{if(!s.set.title.trim()){p("\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043D\u0430\u0437\u0432\u0430\u043D\u0438\u0435 \u043D\u0430\u0431\u043E\u0440\u0430");return}s.editIdx===-1?s.sets.push(s.set):s.sets[s.editIdx]=s.set,M(s.sets),s.view="list",E(e)}),e.querySelector("#ce-del-set")?.addEventListener("click",()=>{p(`\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u043D\u0430\u0431\u043E\u0440 \xAB${d(s.set.title)}\xBB?`,()=>{s.sets.splice(s.editIdx,1),M(s.sets),s.view="list",E(e)})}),e.querySelector("#ce-add-c")?.addEventListener("click",()=>{s.set.characters.push({name:"",cost:0,imageUrl:""}),s.expandedC=s.set.characters.length-1,S(e),setTimeout(()=>{e.querySelector(`[data-crow="${s.expandedC}"]`)?.scrollIntoView({behavior:"smooth",block:"center"})},50)}),me(e)}function ve(e){return e.length===0?'<div class="text-gray-500 text-center py-8 text-sm">\u041D\u0435\u0442 \u043F\u0435\u0440\u0441\u043E\u043D\u0430\u0436\u0435\u0439. \u041D\u0430\u0436\u043C\u0438\u0442\u0435 \xAB+ \u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C\xBB.</div>':e.map((t,r)=>{let a=s.expandedC===r,n=t.imageUrl?b(t.imageUrl):"";return`
  ...
  `,e.querySelector("#btn-new")?.addEventListener("click",()=>T("setup")),e.querySelector("#btn-continue")?.addEventListener("click",()=>qe()),e.querySelector("#btn-pack-ed")?.addEventListener("click",()=>T("pack-editor")),e.querySelector("#btn-char-ed")?.addEventListener("click",()=>T("character-editor"))}function qe(){if(document.getElementById("save-picker-overlay")?.remove(),f().length===0){_();return}let t=document.createElement("div");t.id="save-picker-overlay",t.className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4";let r=()=>{let n=f();return n.length===0?'<div class="text-gray-500 text-center py-8 text-sm">\u041D\u0435\u0442 \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u0438\u0439</div>':n.map(l=>`
  ...
  `;let a=()=>{t.querySelectorAll("[data-resume]").forEach(n=>{n.addEventListener("click",()=>{t.remove(),ne(n.dataset.resume)})}),t.querySelectorAll("[data-delete]").forEach(n=>{n.addEventListener("click",()=>{p("\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u044D\u0442\u043E \u0441\u043E\u0445\u0440\u0430\u043D\u0435\u043D\u0438\u0435?",()=>{H(n.dataset.delete);let l=t.querySelector("#sp-list");l&&(l.innerHTML=r()),a(),f().length===0&&(t.remove(),_())})})})};a(),t.querySelector("#sp-close")?.addEventListener("click",()=>t.remove()),t.addEventListener("click",n=>{n.target===t&&t.remove()}),document.body.appendChild(t)}async function Ce(){te(T),le(T),await re(),T("home")}document.addEventListener("DOMContentLoaded",()=>{Ce()});})();
